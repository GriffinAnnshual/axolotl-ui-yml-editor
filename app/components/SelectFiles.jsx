...
export const SelectFiles = ({setJson}) => {
    ...
    setSecond([])
    setThird([])
    setSelectedFile({...selectedFile, first: selectedLanguage.label , second: "", third: ""})
		if (selectedLanguage) {
    ...
    const selectedLanguage = files.find((items)=> items.label === selectedLabel)
    if (!selectedLanguage) {
				setThird([])
			}
    if (selectedLanguage){
	...
	return (
		<div className="flex pt-10 pl-10">
			<select
				...
				className="text-black rounded-lg mr-2 px-2">
				<option
					value=""
					selected>
					Select a option
			...
			{submit && (
				<div
					onClick={handleLoad}
					className="w-fit  cursor-pointer p-1 px-2 rounded-md ml-4 bg-white text-black">
					Load
				</div>
			)}
			{!submit && (
				<div
					className="w-fit p-1 px-2 rounded-md ml-4 bg-white opacity-50 text-black">
					Load
				</div>
			)}
		</div>
